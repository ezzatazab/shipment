...
def execute(filters=None):
	...
	if filters.get("group_by") == "Group by Party" and filters.get("party"):
		unique_rows = []
		seen = set()
		for r in res:
...
def set_customs_shipment_details(gl_entries):
	"""
	Fetch policy_number and customs_declaration from Customs Shipment
	for Sales Invoice vouchers.
	"""
	# Get all Sales Invoice voucher numbers
	sales_invoice_vouchers = [
		gl.get("voucher_no") 
		for gl in gl_entries 
		if gl.get("voucher_type") == "Sales Invoice" and gl.get("voucher_no")
	]
	
	if not sales_invoice_vouchers:
		# Initialize fields for all entries
		for gl in gl_entries:
			gl["policy_number"] = ""
			gl["customs_declaration"] = ""
			gl["vehicle_info"] = ""
		return
	
	# Get unique Sales Invoice vouchers
	sales_invoice_vouchers = list(set(sales_invoice_vouchers))
	
	# Fetch customs_shipment from Sales Invoice Items
	customs_shipment_map = {}
	for si_name in sales_invoice_vouchers:
		customs_shipment_items = frappe.get_all(
			"Sales Invoice Item",
			filters={"parent": si_name, "customs_shipment": ("!=", "")},
			fields=["customs_shipment"],
			order_by="idx asc",
			limit=1
		)
		if customs_shipment_items and customs_shipment_items[0].get("customs_shipment"):
			customs_shipment_map[si_name] = customs_shipment_items[0].get("customs_shipment")
	
	# Fetch policy_number, customs_declaration, and shipment_type from Customs Shipment
	customs_details = {}
	if customs_shipment_map:
		customs_shipment_names = list(set(customs_shipment_map.values()))
		for cs_name in customs_shipment_names:
			...
			if details:
				customs_details[cs_name] = details
				
				# Fetch truck_names or vessel_no based on shipment_type
				shipment_type = details.get("shipment_type")
				vehicle_info = []
				
				if shipment_type == "Truck":
					# Get truck names from truck_details child table
					truck_details = frappe.get_all(
						"Shipment Truck Detail",
						filters={"parent": cs_name, "parenttype": "Customs Shipment"},
						fields=["truck_name"],
						order_by="idx asc"
					)
					vehicle_info = [truck.get("truck_name") for truck in truck_details if truck.get("truck_name")]
					
				elif shipment_type == "Container":
					# Get vessel_no from vessel_details child table
					vessel_details = frappe.get_all(
						"Shipment Vessel Detail",
						filters={"parent": cs_name, "parenttype": "Customs Shipment"},
						fields=["vessel_no"],
						order_by="idx asc"
					)
					vehicle_info = [vessel.get("vessel_no") for vessel in vessel_details if vessel.get("vessel_no")]
				
				# Join with comma
				customs_details[cs_name]["vehicle_info"] = ", ".join(vehicle_info) if vehicle_info else ""
	
	# Set policy_number, customs_declaration, and vehicle_info for each GL entry
	for gl in gl_entries:
		gl["policy_number"] = ""
		gl["customs_declaration"] = ""
		gl["vehicle_info"] = ""
		...
		if gl.get("voucher_type") == "Sales Invoice" and gl.get("voucher_no"):
			si_name = gl.get("voucher_no")
			customs_shipment = customs_shipment_map.get(si_name)
			if customs_shipment and customs_shipment in customs_details:
				details = customs_details[customs_shipment]
				gl["policy_number"] = details.get("policy_number") or ""
				gl["customs_declaration"] = details.get("customs_declaration") or ""
				gl["vehicle_info"] = details.get("vehicle_info") or ""
...
def get_data_with_opening_closing(filters, account_details, accounting_dimensions, gl_entries):
	...
	set_customs_shipment_details(gl_entries)
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
	...
	def update_value_in_dict(data, key, gle):
		...
		# Preserve policy_number, customs_declaration, and vehicle_info from gle if not already set in data[key]
		if key not in ["opening", "closing", "total"]:
			if not data[key].get("policy_number") and gle.get("policy_number"):
				data[key]["policy_number"] = gle.get("policy_number")
			if not data[key].get("customs_declaration") and gle.get("customs_declaration"):
				data[key]["customs_declaration"] = gle.get("customs_declaration")
			if not data[key].get("vehicle_info") and gle.get("vehicle_info"):
				data[key]["vehicle_info"] = gle.get("vehicle_info")
...
def get_columns(filters):
	...
	columns += [
		...
		{"label": _("Policy Number"), "fieldname": "policy_number", "fieldtype": "Data", "width": 120},
		{"label": _("Customs Declaration"), "fieldname": "customs_declaration", "fieldtype": "Data", "width": 150},
		{"label": _("Truck Name / Vessel No"), "fieldname": "vehicle_info", "fieldtype": "Data", "width": 200},
